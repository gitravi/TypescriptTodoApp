...
app.factory("todoFactory", ["$firebaseArray", ($firebaseArray) => {
    var ref = new Firebase("https://amber-torch-288.firebaseio.com/ToDOApp");
    return $firebaseArray(ref);
}]);


app.controller("ToDoAppController", ["$scope", "todoFactory", ($scope, todoFactory)=> {
    $scope.newTodo = "";
    ...
    $scope.selectAll = function () {
        angular.forEach($scope.todos,(i) => { i.isDone = $scope.masterCheckBoxStatus; $scope.todos.$save(i); });
        return true;
    }
